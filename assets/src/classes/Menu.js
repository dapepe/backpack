...
Backpack.Menu = new Class({
  ...
  initialize: function(parent, options) {
    ...
    this._ui.btnGroup = new Element('div', {
      'class': 'btn-group',
      'styles': {
        'opacity': 0
      }
    });
    this._ui.btnBack = new Element('button', {
      'class': 'btn btn-xs btn-default',
      'html': 'History'
    });
    this._ui.btnToggle = new Element('button', {
    ...
    this._ui.btnNew = new Element('button', {
      'class': 'btn btn-xs btn-primary mtop-10 mright-10 right',
      'styles': {
        'opacity': 0
      },
  ...
  updateFramePos: function() {
    ...
    this._ui.frame.setStyle('margin-left', this._menuFrames.length <= 1
      ? 0
      : ((this._menuFrames.length - 1) * -100) + '%');
  },
