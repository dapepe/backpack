...
Backpack.Reference = new Class({
	...
	initialize: function (display, collection, options) {
		...
		try {
			...
			this._display.textbox = new Element('div', {
                'class'      : 'form-control'
			})
    ...
    initFinder: function(callback, callbackProperties) {
        ...
        this.api = new Backpack.Api(this._collection, {
            onReady: function(schema) {
                this._schema = schema;
                this._searchbox = new Element('input', {'class': 'form-control', 'placeholder': 'Search'})
                ...
                this._finder = new Backpack.Finder({
                    'header': this._searchbox
                });
                this._popup = new gx.bootstrap.Popup({
                    ...
                    'footer': [
                        __({'tag': 'button', 'class': 'btn btn-danger left', 'html': '<span class="glyphicon glyphicon-trash mright-10"></span>Remove reference', 'onClick': function() {
                            this.setValue(null);
                            this._popup.hide();
                        }.bind(this)}),
                        __({'tag': 'button', 'class': 'btn btn-primary', 'html': '<span class="glyphicon glyphicon-remove mright-10"></span>Close', 'onClick': function() {
                            this._popup.hide();
                        }.bind(this)})
                    ],
                    'onOpen': function() {
            ...
            function (docs) {
                ...
                docs.each(function (doc) {
                    var item = this._finder.addItem(doc.identifier, {
                        ...
                        'showPublic': this._schema.published == null ? null : doc.published,
                        'onClick': function () {
                            this.setValue(doc);
                        }.bind(this)
                    });
     ...
     */
    getValue: function () {
        return this._doc == null ? null : this._doc._id;
    },
        ...
        switch (typeOf(value)) {
            ...
            case 'object':
                ...
                this.updateLink(new Element('a', {'html': this._doc.identifier}).addEvent('click', function() {
                    app.gui.openEditor(this._doc, this._schema.contenttype, this._collection, {
                        'title': this._doc.identifier
                    });
                }.bind(this)));
                break;
            case 'string':
                ...
                this.api.identify(value, function(doc) {
                    this.setValue(typeOf(doc) == 'object' ? doc : null);
                }.bind(this));
                return;
            default:
