...
web build:
 - http://mootools.net/core/cd2402d0ec31e331b4774b1346a4e875

packager build:
 - packager build Core/Core Core/Array Core/String Core/Element Core/Element.Style Core/Element.Event Core/Element.Delegation Core/Element.Dimensions Core/Fx Core/Fx.CSS Core/Fx.Tween Core/Fx.Morph Core/Fx.Transitions Core/DOMReady
...
};}});Number.implement(a);})(["abs","acos","asin","atan","atan2","ceil","cos","exp","floor","log","max","min","pow","sin","sqrt","tan"]);(function(){var f=this.document;
var d=f.window=this;var a=function(k,e){k=k.toLowerCase();e=(e?e.toLowerCase():"");var l=k.match(/(opera|ie|firefox|chrome|trident|crios|version)[\s\/:]([\w\d\.]+)?.*?(safari|(?:rv[\s\/:]|version[\s\/:])([\w\d\.]+)|$)/)||[null,"unknown",0];
...
}})();(function(){var c={};var b=function(e){var f;if(e.wheelDelta){f=e.wheelDelta%120==0?e.wheelDelta/120:e.wheelDelta/12;}else{var d=e.deltaY||e.detail||0;
f=-(d%3==0?d/3:d*10);}return f;};var a=this.DOMEvent=new Type("DOMEvent",function(d,h){if(!h){h=window;}d=d||h.event;if(d.$extended){return d;}this.event=d;
this.$extended=true;this.shift=d.shiftKey;this.control=d.ctrlKey;this.alt=d.altKey;this.meta=d.metaKey;var j=this.type=d.type;var i=d.target||d.srcElement;
while(i&&i.nodeType==3){i=i.parentNode;}this.target=document.id(i);if(j.indexOf("key")==0){var e=this.code=(d.which||d.keyCode);this.key=c[e];if(j=="keydown"||j=="keyup"){if(e>111&&e<124){this.key="f"+(e-111);
}else{if(e>95&&e<106){this.key=e-96;}}}if(this.key==null){this.key=String.fromCharCode(e).toLowerCase();}}else{if(j=="click"||j=="dblclick"||j=="contextmenu"||j=="wheel"||j=="DOMMouseScroll"||j.indexOf("mouse")==0){var k=h.document;
k=(!k.compatMode||k.compatMode=="CSS1Compat")?k.html:k.body;this.page={x:(d.pageX!=null)?d.pageX:d.clientX+k.scrollLeft,y:(d.pageY!=null)?d.pageY:d.clientY+k.scrollTop};
this.client={x:(d.pageX!=null)?d.pageX-h.pageXOffset:d.clientX,y:(d.pageY!=null)?d.pageY-h.pageYOffset:d.clientY};if(j=="DOMMouseScroll"||j=="wheel"||j=="mousewheel"){this.wheel=b(d);
}this.rightClick=(d.which==3||d.button==2);if(j=="mouseover"||j=="mouseout"){var l=d.relatedTarget||d[(j=="mouseover"?"from":"to")+"Element"];while(l&&l.nodeType==3){l=l.parentNode;
}this.relatedTarget=document.id(l);}}else{if(j.indexOf("touch")==0||j.indexOf("gesture")==0){this.rotation=d.rotation;this.scale=d.scale;this.targetTouches=d.targetTouches;
this.changedTouches=d.changedTouches;var g=this.touches=d.touches;if(g&&g[0]){var f=g[0];this.page={x:f.pageX,y:f.pageY};this.client={x:f.clientX,y:f.clientY};
...
}return this;}});a.defineKey=function(e,d){c[e]=d;return this;};a.defineKeys=a.defineKey.overloadSetter(true);a.defineKeys({"38":"up","40":"down","37":"left","39":"right","27":"esc","32":"space","8":"backspace","9":"tab","46":"delete","13":"enter"});
